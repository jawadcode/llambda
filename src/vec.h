...
#define vec_push_sig(T) void T##Vec_push(T##Vec *array, T value);

#define vec_push(T)                                                            \
  void T##Vec_push(T##Vec *array, T value) {                                   \
    if (array->capacity < array->length + 1) {                                 \
      int old_capacity = array->capacity;                                      \
      array->capacity = GROW_CAPACITY(old_capacity);                           \
      array->buffer =                                                          \
          GROW_ARRAY(T, array->buffer, old_capacity, array->capacity);         \
    }                                                                          \
                                                                               \
...
#define vec_free(T)                                                            \
  void T##Vec_free(T##Vec *array) {                                            \
    FREE_ARRAY(T, array->buffer, array->capacity);                             \
    array->buffer = NULL;                                                      \
    array->capacity = 0;                                                       \
    array->length = 0;                                                         \
  }
